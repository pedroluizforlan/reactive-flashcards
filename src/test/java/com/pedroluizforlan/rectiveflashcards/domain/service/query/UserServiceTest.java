...
import com.pedroluizforlan.rectiveflashcards.core.factorybot.document.UserDocumentFactoryBot;
...
public class UserServiceTest {
    ...
    void whenTryToSaveUserWithExistingEmailThenThrowError(){
        ...
        StepVerifier.create(userService.save(document))
                .verifyError(EmailAlreadyInUsedException.class);
    ...
    private static Stream<Arguments> updateTest(){
        ...
        return Stream.of(
                Arguments.of(document, Mono.empty(), storedDocument),
    ...
    void whenTryToUpdateUserNonStoredThenThrowError(){
        var document = UserDocumentFactoryBot.builder().build();
        when(userQueryService.findByEmail(anyString())).thenReturn(Mono.empty());
